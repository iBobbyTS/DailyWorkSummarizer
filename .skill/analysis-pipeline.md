...
- 分析启动模式有三种：不自动分析只保留手动入口；定时分析只按 `analysisTimeMinutes` 触发；实时分析在截图成功保存后 1 秒触发同一套普通待处理文件夹分析流程。
- “仅在充电时自动分析”只约束自动触发：定时和实时会检查充电器，手动“立即分析”不检查。
- 没有 pending 截图时，触发分析不应创建空的 `analysis_runs`。
- 运行中再次触发分析时，不要取消、暂停或重启当前 run；应把新扫描到的截图路径追加到当前待处理队列末尾，并同步更新 `analysis_runs.total_items`。
- 用户取消分析时，当前 run 应立即停止接收追加；后续触发通过 `pendingTriggerAfterCurrentRun` 在当前 run 结束后重新启动。
- `analysis_results.captured_at` 有唯一约束；重复时间写入应被忽略，删除对应截图文件，不覆盖旧结果。
- 截图分析支持两种远程路径：
