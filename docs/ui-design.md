...
- `Analyze Now` should be disabled while a work-content summary run is active; during a screenshot-analysis run it remains the stop action for that run. Backfill should be disabled while any run is active, using the run coordinator gate rather than only visible progress state so handoff windows cannot enable it briefly. The analysis startup mode submenu remains enabled during runs and only affects future triggers.
